...
function main() {
    ...
    //$.locale = "ru"
    {
        var strMessage = "File collector",
            ...
            strBridgeSelection = { ru: "Список выбранных файлов", en: "Selected files list" },
            ...
            strErrDefalutPreset = { ru: "Используйте другое имя при создании пресета!", en: "Use a different name when creating a preset!" },
            ...
            strMigrateToXML = { ru: 'Скрипт этой версии использует другую систему настроек.\n\nПеренести настройки из прошлой версии?\n\nНовый файл настроек будет доступен по следующему пути:\n', en: 'The script of this version uses a different settings system.\n\nTransfer settings from previous version?\n\nThe new settings file will be available in the following path:\n' }
        cfg = new Config,
            allFiles = {},
            fromBridge = {};
    }
    if (BridgeTalk.appName == "photoshop") {
        ...
        if (f.exists) migrateToXML(f, new File(Folder.userData + "/" + strMessage + ".xml"));
    }
    cfg.getScriptSettings()
    var w = buildWindow(); var result = w.show()
    if (result != 2) { cfg.putScriptSettings() }
    ...
    function buildWindow() {
        ...
        target = etRename
        {
            dlPreset.onChange = function () {
                ...
                cfg.currentPreset = this.selection.text;
                if (renew) {
                    this.selection.index == 0 ?
                        cfg.options = (new Config).options :
                        cfg.options = cfg.getPreset(this.selection.text);
                    w.onShow(true);
                }
                if (w.visible) cfg.putScriptSettings()
                cfg.checkPresetIntegrity(w)
            }
            bnSave.onClick = function () {
                cfg.putPreset(dlPreset.selection.text, cfg.options, "save")
                cfg.checkPresetIntegrity(w)
                ...
                if (nm != null && nm != "") {
                    if (cfg.getPreset(nm) == "" && nm != strPresetDefailt) {
                        ...
                        if (nm != strPresetDefailt) {
                            ...
                            alert(strErrDefalutPreset, strErr, 1)
                        }
                    }
                }
                cfg.putScriptSettings()
            ...
            bnDel.onClick = function () {
                ...
                cfg.putPreset(dlPreset.selection.text, cfg.options, "delete")
                loadPresets()
        ...
        rbFiles.onClick = function () {
            ...
            if (BridgeTalk.appName == "photoshop") {
                ...
                var currentSelection = app.document.selections
                if (currentSelection.length) {
                    currentSelection.exists = true
                    currentSelection.fsName = Folder(app.document.presentationPath).fsName
                }
                enumFiles(currentSelection.length ? currentSelection : Folder(app.document.presentationPath))
            }
            checkButtonsState()
        }
        ...
        ok.onClick = function () {
            ...
            if (result != null) {
                if (BridgeTalk.appName == "photoshop") {
                    ...
                    var d = new progressWindow(strBnSearchRename, result.found.length);
                    moveFiles(result.found, d);
                    w.close();
                }
            }
        }
        ...
        dlMode.onChange = function () {
            ...
            if (w.visible) cfg.checkPresetIntegrity(w)
            formattedText = buildList(sourceText)
            var columns = 0,
                wordLen = {},
        ...
        bnFile.onClick = function () { etRename.textselection = "[N]"; textList.onChange() }
        bnFolder.onClick = function () { etRename.textselection = "[P]"; textList.onChange() }
        bnHeader.onClick = function () { etRename.textselection = "[H]"; textList.onChange() }
        etRename.onChanging = function () { textList.onChange(); }
        etSearch.onChanging = function () { textList.onChange(); }
        bnFillList.onClick = function () {
            ...
            for (var a in allFiles) {
                if (!cfg.globalMode) progress.updateProgress()
            ...
            dlMode.selection = 0
            textList.selection = 0
            if (!cfg.globalMode) progress.close();
            checkButtonsState()
        }
        dlFilter.onChange = function () {
            if (cfg.globalMode) {
                ...
                cfg.fileFilter = typesArray[this.selection == null ? 0 : this.selection.index]
            }
        }
        ...
        w.onShow = function (fromPreset) {
            ...
            if (!fromPreset) {
                ...
                loadPresets()
                dlPreset.selection = dlPreset.find(cfg.currentPreset) != null ? dlPreset.find(cfg.currentPreset) : 0
            }
            dlMode.selection = cfg.listMode
        ...
        function changeGlobalMode(mode) {
            ...
            dlFilter.removeAll();
            if (textList.length) textList.onClick();
            if (mode) {
                ...
                if (currentFolder.exists) {
                    ...
                    if (BridgeTalk.appName == "photoshop") {
                        ...
                        if (currentFolder instanceof Folder) {
                            findAllFiles(cfg.sourcePath, allFiles, cfg.useSubfolders)
                        } else {
                            if (currentFolder.length) {
                                for (var i = 0; i < currentFolder.length; i++) {
                                    if (currentFolder[i].type == "file") {
                                        putFileToTypeObject(currentFolder[i].spec, allFiles)
                                    } else {
                                        findAllFiles(currentFolder[i].spec.fsName, allFiles, cfg.useSubfolders)
                                    }
                                }
                            }
                        }
        ...
        function moveFiles(fileList, progress) {
            var f = [];
    ...
    function searchWindow(s, h) {
        ...
        w.onShow = function () {
            if (cfg.globalMode) {
                ...
                if (BridgeTalk.appName == "photoshop") {
                    ...
                    findFile(s, h, allFiles, cfg.fileFilter, result)
                }
            }
            renewList()
        }
        ...
        function generateLayerNames(f) {
            ...
            for (var i = 0; i < len; i++) {
                for (var x = i + 1; x < len; x++) {
                    if (!f[x].targetName) continue;
                    if (f[i].source.id == f[x].source.id && f[i].source.parentID == f[x].source.parentID) {
                        var cur = f[x].targetName.replace(new RegExp(' ?' + f[x].source.name + ' ?', 'g'), '');
            ...
            for (var i = 0 + cfg.listMode; i < listLen; i++) {
                ...
                if (curSearch.length) {
                    for (var n = 0; n < curSearch.length; n++) {
                        ...
                        if (currentResult.length > 0) {
                            currentResult.sort(sortFiles)
                            var idx = currentResult[0].index
                            for (var x = 0; x < currentResult.length; x++) {
                                if (currentResult[x].index == idx) found.push(currentResult[x])
                            }
                        } else {
                            notFound.push({ name: s[i].join(' '), search: curSearch[n].text })
                        }
    ...
    function Config() {
        ...
        this.getScriptSettings = function () {
            var f = new File(Folder.userData + "/" + strMessage + ".xml");
            try {
                if (f.exists) {
                    f.open('r')
                    f.encoding = 'UTF8'
                    objectFromXML(this, new XML(f.read()))
                    f.close();
                }
            } catch (e) { alert(e, strErr, 1) }
            function objectFromXML(settingsObj, s) {
                for (var a in settingsObj) {
                    switch (String(s[a].type)) {
                        case 'string': settingsObj[a] = String(s[a].text()); break;
                        case 'number': settingsObj[a] = Number(String(s[a].text())); break;
                        case 'boolean': settingsObj[a] = Boolean(Number(String(s[a].text()))); break;
                        case 'object':
                            var len = s[a].preset.length();
                            settingsObj[a] = {};
                            for (var i = 0; i < len; i++) {
                                settingsObj[a][String(s[a].preset[i].text())] = String(s[a].preset[i]['value'])
                            }
                            break;
                    }
                }
        ...
        this.putScriptSettings = function () {
            var s = objectToXML(this),
                f = new File(Folder.userData + "/" + strMessage + ".xml");
            try {
                f.open('w')
                f.encoding = 'UTF8'
                f.write(s.toXMLString())
                f.close()
            } catch (e) { alert(e, strErr, 1) }
            function objectToXML(o) {
                s = new XML('<settings></settings>');
                for (var a in o) {
                    switch (typeof (o[a])) {
                        case 'string': s[a] = o[a]; s[a]['type'] = 'string'; break;
                        case 'number': s[a] = o[a]; s[a]['type'] = 'number'; break;
                        case 'boolean': s[a] = Number(o[a]); s[a]['type'] = 'boolean'; break;
                        case 'object':
                            var counter = 0;
                            s[a] = ''
                            s[a]['type'] = 'object';
                            for (var n in o[a]) {
                                s[a]['preset'][counter] = n;
                                s[a]['preset'][counter].value = o[a][n]
                                counter++;
                            }
                            break;
                    }
        ...
        this.getPresetList = function () {
            var output = []
            for (var a in this.presets) output.push({ key: a, val: this.presets[a] })
        ...
        this.getPreset = function (key) {
            return this.presets[key] ? this.presets[key] : ''
        }
        this.checkPresetIntegrity = function (window) {
            ...
            if (dlPreset.selection.index > 0) {
                ...
                var old = this.getPreset(dlPreset.selection.text)
        ...
        this.putPreset = function (key, val, mode) {
            ...
            this.presets = {}
            for (var i = 0; i < output.length; i++) this.presets[output[i].key] = output[i].val
    ...
    function migrateToXML(f, f1) {
        if (!f1.exists && confirmBox(strMigrateToXML + f1.fsName, 0, strMessage)) {
            ...
            if (d.count) descriptorToObject(tempConfig, d)
            try {
                var d = app.getCustomOptions('FileCollector');
                for (var i = 0; i < d.count; i++) tempConfig.presets[typeIDToStringID(d.getKey(i))] = d.getString(d.getKey(i)).replace(new RegExp('\v.'), '')
            } catch (e) { }
            tempConfig.putScriptSettings();
        }
